...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    let result: u64 = zip(first_row_sorted, second_row_sorted)
        .fold(0, |acc, (x, y)| acc + if x > y { x - y } else { y - x });
...
pub fn part_two(input: &str) -> Option<u64> {
    ...
    let result = first_row.fold(0, |acc, x| {
        acc + match count_table.get(&x) {
            None => 0,
            Some(val) => val * x,
        }
    });
